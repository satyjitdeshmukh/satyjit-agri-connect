...
const UserProfile = () => {
  ...
  const fetchUser = async () => {
    await axios.get(`http://localhost:4300/user?email=${encodeURIComponent(userEmail)}`)
      .then((res) => {
  ...
  const fetchOrdersByUser = async () => {
    await axios.get(`http://localhost:4300/orders?email=${encodeURIComponent(userEmail)}`)
      .then((res) => {
        // console.log(res.data)
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    await axios.post('http://localhost:4300/user', 
      {
