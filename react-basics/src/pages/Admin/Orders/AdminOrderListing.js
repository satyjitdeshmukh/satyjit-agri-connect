...
const AdminOrderListing = () => {
  ...
  useEffect(() => {
    fetchOrders('http://localhost:4300/orders/pending', setPendingOrders, 'pendingOrders');
    fetchOrders('http://localhost:4300/orders/confirmed', setConfirmedOrders, 'confirmedOrders');
    fetchOrders('http://localhost:4300/orders/canceled', setCanceledOrders, 'canceledOrders');
  }, []);
  ...
  const handleConfirm = async (orderId) => {
    try {
      await axios.patch(`http://localhost:4300/orders/confirm?transactId=${orderId}`)
        .then((res) => {
          console.log(res.data);
